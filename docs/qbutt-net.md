...
For UDP, the first authenticated local SOCKS UDP association that sends while a UDP lease is active becomes that path's gateway association. Its payloads cross the fragmented QUIC channel and public replies return as ordinary SOCKS UDP responses with the original source endpoint. The effective payload limit at this SOCKS boundary is 65,485 bytes, leaving room for the largest numeric IPv6 source envelope inside one legal UDP datagram; an oversized packet retires the association. The gateway carrier itself retains its 65,507-byte transport limit. A second simultaneous local UDP association is closed instead of silently bypassing the gateway. Starting a UDP gateway retires existing direct UDP associations. Gateway expiry or carrier failure retires its bound association and rejects replacements until the parent advances the path generation, so a lease failure cannot change routes in place. A TCP-only gateway leaves ordinary adapter UDP unchanged. Reflection, rate, replay and reassembly policy stays owned by qbutt-gateway.
